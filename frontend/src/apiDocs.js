...
export const GROUPS = [
  ...
  {
    id: 'vsl',
    title: 'VSL watch time',
    blurb:
      'How much of the sales video each lead actually watched. Reads a SECOND, read-only Mongo ' +
      'cluster owned by the VSL landing-page project and joins it to follow-ups on the last 10 ' +
      'digits of the phone. Not admin-only: a rep is scoped to VSL leads matching follow-ups they ' +
      'own, so they never see another rep\'s book — or a watcher who is not in the dashboard at all. ' +
      'With VSL_MONGO_URI unset every endpoint answers `configured: false` rather than failing.',
    endpoints: [
      {
        method: 'GET',
        path: '/api/vsl/leads',
        auth: USER,
        summary: 'VSL leads in a date range with peak watch time, lead source and follow-up link.',
        source: 'backend/modules/vsl/controllers/vslController.js',
        note:
          '`watch.seconds` is the PEAK ever recorded in the event log, not the VSL lead record\'s own ' +
          'figure — that one is overwritten on every beacon and so falls when somebody reopens the ' +
          'video. `watch.basis` says which you got: `events` (measured), `lead` (the overwritable ' +
          'value, for rows with no events) or `none`. `totals` is computed before the optional ' +
          'filters, so the summary cards keep showing the whole range.',
        query: [
        ...
        response: {
          ...
          count: 214,
          truncated: false,
          totals: { all: 214, watched: 88, played: 121, opened: 160, sent: 214, notInDashboard: 12, unjoinable: 3, minutesTotal: 1042.5 },
          data: [
            {
              leadId: '823c0a57…',
              phone: '919876543210',
              ...
              openCount: 3,
              watch: { seconds: 752, minutes: 12.5, percentage: 45, videoDuration: 1670, events: 41, completed: false, basis: 'events' },
              engagement: 'watched',
              leadSource: { key: 'Meta Ads', raw: 'ig', basis: 'deal' },
              dashboard: { linked: true, taskId: '9876543210', contactName: 'Rahul S', ownerName: 'Veera', ownerEmail: 'veera@focasedu.com', status: 'In Progress' },
            },
          ],
        },
        errors: [['400', "Invalid 'from' date: expected YYYY-MM-DD"]],
      },
      {
        method: 'GET',
        path: '/api/vsl/status',
        auth: USER,
        summary: 'Whether the VSL cluster is wired up, and whether the watch map is warm.',
        source: 'backend/modules/vsl/controllers/vslController.js',
        note: '`peaks` is admin-only and is omitted — not nulled — for a rep.',
        response: {
          success: true,
          configured: true,
          db: 'focas',
          peaks: { leads: 1420, phoneKeys: 1388, builtAt: '2026-08-16T11:00:00.000Z', buildMs: 3120, stale: false, error: null },
        },
      },
    ],
  },
